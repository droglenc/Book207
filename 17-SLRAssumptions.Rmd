...
```{r FLPResid1, echo=FALSE, fig.width=7, fig.cap="Scatterplot with best-fit line and four points highlighted (Left) and residual plot with same four points highlighted (Right)."}
...
flp1 <- ggplot(data=tmp,mapping=aes(x=x,y=y,label=lbl)) +
  geom_point(pch=21,color="black",bg="lightgray") +
  geom_point(data=filter(tmp,lbl!=""),mapping=aes(x=x,y=y),color="red") +
  ggrepel::geom_text_repel(min.segment.length=0.5,box.padding=1,max.overlaps=Inf,force=5,
                           arrow=arrow(length=unit(1.5,"mm"),type="closed"),
                           size=lbl_text_size,color="red",segment.color="black") +
  geom_smooth(method="lm",se=FALSE) +
  labs(x="Explanatory Variable",y="Response Variable") +
  theme_NCStats() +
...
residp1 <- ggplot(data=tmp,mapping=aes(x=p,y=r,label=lbl)) +
  geom_point(pch=21,color="black",bg="lightgray") +
  geom_point(data=filter(tmp,lbl!=""),mapping=aes(x=p,y=r),color="red") +
...
```{r FLPResid2, echo=FALSE, fig.width=7, fig.cap="Scatterplot with best-fit line and four points highlighted (Left) and residual plot with same four points highlighted (Right)."}
y2a <- -slp*x2+int+e2
lm2a <- lm(y2a~x2)
p2a <- predict(lm2a)
r2a <- lm2a$residuals
...
## Homoscedasticity
Strictly speaking "homoscedasticity" means that the the variances of $Y$ at each given value of $X$ are equal. Seldom will you have enough multiple observations at the same value of $X$ to test this explicitly. Thus, in practice, homoscedasticity translates into assessing whether the same general variance or scatter of points exists around the line for all values of $X$. Heteroscedasticity, or non-constant variance around the line, will generally appear as a cone or funnel shape on a residual plot (Figure \@ref(fig:SLRAssumptionsHetero)). Sometimes the funnel-shape may be difficult to see if the linearity assumption is also not met (Figure \@ref(fig:SLRAssumptionsHetero)-Right). Make sure to compare these residual plots to Figure \@ref(fig:SLRAssumptionsMet).
...
```{r SLRInfluentialPoint, echo=FALSE, fig.width=7, fig.cap="Scatterplot with a best-fit line for all points (dashed red) and all points excluding the red point (solid black) (Left) and the residual plot from the fit to all points (Right)."}
...
flpInfl <- ggplot() +
  ...
  geom_point(data=Infl2,mapping=aes(x=x,y=y),pch=21,color="black",bg="lightgray") +
  geom_smooth(data=Infl2,mapping=aes(x=x,y=y),method="lm",se=FALSE) +
  labs(x="Explanatory Variable",y="Response Variable") +
  theme_NCStats() +
  theme(axis.text=element_blank(),aspect.ratio=1)
residInfl <- ggplot() +
  ...
  geom_point(data=Infl2,mapping=aes(x=p,y=r),pch=21,color="black",bg="lightgray") +
  labs(x="Predicted Values",y="Residuals") +
  theme_NCStats() +
  theme(axis.text=element_blank(),aspect.ratio=1)
flpInfl + residInfl
