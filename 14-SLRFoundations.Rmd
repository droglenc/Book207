...
knitr::kable(ev,booktabs=TRUE,caption="Actual air temperature (`MeanAirTemp`) and altitude lapse rate (`Altitude`) data for Mount Everest in the winter.") %>%
...
::: {.tip data-latex=""}
Remember that statistics like $\hat{\alpha}$ and $\hat{\beta}$ are subject to sampling variability (i.e., vary from sample to sample) and have corresponding standard errors (to be shown in Module \@ref(SLRInference)).
:::
...
slr <- ggplot(data=ev2,mapping=aes(x=Altitude,y=MeanAirTemp)) +
  ...
  theme_NCStats()
slrp <- slr +
  geom_point(data=ev2[2,],mapping=aes(x=Altitude,y=MeanAirTemp),color="red",size=2) +
  ...
  annotate(geom="text",x=3000,y=-5,label=expression(x[2]),parse=TRUE,
           size=lbl_text_size,vjust=0.25,hjust=-0.25) +
  ...
  annotate(geom="text",x=2700,y=1,label=expression(mu[Y*"|"*X==x[2]]),parse=TRUE,
           size=lbl_text_size,vjust=1,hjust=-0.1) +
  annotate(geom="segment",x=2700,y=1,xend=-Inf,yend=ev2$pMeanAirTemp[2],
           arrow=arrow(length=unit(1.5,"mm"),type="closed"))
slrp
```

As noted in previous modules, the residual is the vertical difference between an individual's observed value of the response variable (i.e., $y_{i}$) and a predicted value of the response variable based on the equation of a line and the individual's value of the explanatory variable (i.e., $\hat{\mu}_{Y|X=x_{i}}$). Thus, a residual is $y_{i}-\hat{\mu}_{Y|X=x_{i}}$ and measures how close the line is to that point. A residual for the second individual is demonstrated in Figure \@ref(fig:SLRResidDemo).
...
Based on previous modules, an overall measure of the lack-of-fit of the line to the data is the sum of all squared residuals or the residual sum-of-squares (RSS); i.e., 
...
```{r DemoBFLandRSS, echo=FALSE, fig.width=7, animation.hook="gifski", interval=0.5, fig.cap="Scatterplot with candidate best-fit lines (blue line) and residuals (vertical red dashed lines) in left pane and the residual sum-of-squares for all candidate lines (gray) with the current line highlighted with a red dot. Note how the candidate line is on the best-fit line when the RSS is smallest."}
set.seed(1070)
...
RSSer <- function(res,rss,i) {
  ...
  pres <- ggplot(res,aes(x=X,y=Y)) +
    ...
    geom_path(aes(y=pred),color="blue",size=1.25) +
    theme_NCStats() +
  ...
  prss <- ggplot(rss,aes(x=slope,y=RSS)) +
    geom_path(size=1.25,color="gray30") +
...
Mathematical statisticians have proven that the $\hat{\alpha}$ and $\hat{\beta}$ that minimize the RSS are given by
...
## Best-Fit Line in R
The data for Mount Everest temperature data is loaded below and restricted to just the data from Winter.
...
A best-fit regression line is obtained using the familiar `lm()` with a formula of the form `response~explanatory` and the data frame in `data=`. As usual, the results of this function should be assigned to an object that can be given to other functions to extract specific results.

```{r}
lm1.ev <- lm(MeanAirTemp~Altitude,data=ev)
```

The estimated intercept and slope (i.e., the $\hat{\alpha}$ and $\hat{\beta}$) and corresponding 95% confidence intervals are extracted from the `lm()` object with `coef()` and `confint()`, respectively. I like to "column bind" these two results together to make a synthetic summary table.
...
These results show that the slope of the best-fit line is `r formatC(cf.ev[2],format="f",digits=4)` (95% CI: `r formatC(ci.ev[2,1],format="f",digits=4)` - `r formatC(ci.ev[2,2],format="f",digits=4)`) and the intercept is `r formatC(cf.ev[1],format="f",digits=2)` (95% CI: `r formatC(ci.ev[1,1],format="f",digits=2)` - `r formatC(ci.ev[1,2],format="f",digits=2)`). Thus, the equation of the best-fit line is Mean Air Temperature = `r formatC(cf.ev[1],format="f",digits=2)` - `r formatC(-1*cf.ev[2],format="f",digits=4)`Altitude Lapse Rate. From this slope, it appears that the actual air temperature will decrease by `r formatC(-1*cf.ev[2],format="f",digits=4)`, on average, for each 1 C/km increase in the altitude lapse rate.
...
The best-fit line can be visualized by creating a scatterplot with `ggplot()` and then using `geom_smooth(method="lm",se=FALSE)` to superimpose the best-fit line.
