...
This is called the **ultimate full model** because the model cannot get more complicated that this for this situation as all variables of interest are included in the model. In subsequent modules, simpler models with fewer variables (and parameters) than this one will also be considered.
...
Specific parameters are used in the ultimate full model of an IVR as shown above. Specifically, they are as follows:

* $\alpha$: an overall intercept term.
* $\beta$: coefficient on the covariate.
* $\delta_{i}$: coefficient on the ith indicator variable.
* $\gamma_{i}$: coefficient on the interaction between the covariate and the ith indicator variable.
...
\begin{split}
  \mu_{MIREX|WEIGHT,COHO} &= \alpha+\beta WEIGHT+\delta_{1}COHO+\gamma_{1}WEIGHT:COHO \\
  &=\alpha+\beta WEIGHT+\delta_{1}0+\gamma_{1}WEIGHT:0\\
  &=\alpha+\beta WEIGHT+0+0\\
  &=\alpha+\beta WEIGHT\\
\end{split}
$$
...
\begin{split}
  \mu_{MIREX|WEIGHT,COHO} &= \alpha+\beta WEIGHT+\delta_{1}COHO+\gamma_{1}WEIGHT:COHO \\
  &=\alpha+\beta WEIGHT+\delta_{1}1+\gamma_{1}1WEIGHT \\
  &=\alpha+\beta WEIGHT+\delta_{1}+\gamma_{1}WEIGHT \\
  &=\alpha+\delta_{1}+\beta WEIGHT+\gamma_{1}WEIGHT \\
...
These sub-models can be appended to the tables from Module \@ref(IVRVariables) to provide a succinct summary.
...
Examination of the sub-models for each group shows that each sub-model is itself an SLR model. The sub-model for Chinook Salmon is an SLR with a slope of $\beta$ and an intercept of $\alpha$. In contrast, the sub-model for Coho Salmon is an SLR with a slope of $\beta+\gamma_{1}$ and an intercept of $\alpha+\delta_{1}$.

::: {.tip data-latex=""}
Each sub-model is an SLR for one of the groups represented in the IVR ultimate full model.
:::

The process is similar when there are more than two groups, except that 0s and 1s must be substituted for multiple indicator variables. For example, the ultimate full model for the situation when three years (1977, 1982, and 1986) were considered is
...
To find the sub-model for 1982, for example, requires plugging 1 in for $YEAR1982$ and 0 for $YEAR1986$ and simplifying
...
However, what are $\delta_{i}$ and $\gamma_{i}$? To answer this, consider the following algebra for the difference in intercepts between the $i$th group and the reference group.

$$
\begin{split}
i\text{th Group's Intercept} - \text{Reference Group's Intercept} &= (\alpha+\delta_{i}) - \alpha \\
&= \delta_{i}
\end{split}
$$
...
$$
\begin{split}
i\text{th Group's Slope} - \text{Reference Group's Slope} &= (\beta+\gamma_{i}) - \beta \\
&= \gamma_{i}
\end{split}
$$
...
* $\alpha$: y-intercept term for the reference group.
...
```{r IVRCoefficients, echo=FALSE, warning=FALSE, fig.cap="Representation of two sub-models fit with the ultimate full model in an IVR with the geometric meaning of each parameter. The red line is the sub-model for the reference group. Note that $\\alpha$=2, $\\beta$=0.4, $\\delta_{1}$=-1.6, and $\\gamma_{1}$=0.45 in this example."}
a <- 2
b <- 0.4
d1 <- -1.6
g1 <- 0.45
...
The ultimate full model of an IVR can represent four situations between two groups depending on values of the $\delta_{1}$ and $\gamma_{1}$ parameters (Figure \@ref(fig:IVRSituations)).
...
Y2C <- a+(b+0.25)*x
Y2D <- (a-0.5)+(b+0.25)*x
d <- tibble(sit=rep(c("Coincident Lines","Parallel Lines","Same Intercept","Completely Separate Lines"),each=4),
            group=c(rep(c("A","B","A","B"),each=2),rep(c("A","B","A","B"),each=2)),
            x=rep(x,8),
            y=c(Y1A,Y1B,Y1A,Y2B,Y1A,Y2C,Y1A,Y2D)
            ) %>%
  mutate(sit=factor(sit,levels=c("Coincident Lines","Parallel Lines","Same Intercept","Completely Separate Lines")))
ggplot(data=d,mapping=aes(x=x,y=y,color=group)) +
  geom_line(size=1) +
  facet_wrap(vars(sit)) +
...
Fitting ultimate full model in R is straightforward because it is essentially the same as fitting a Two-Way ANOVA with `lm()` (see \@ref(model-fitting-in-r)) and R automatically creates the indicator variables behind the scenes. The ultimate full model for an IVR is fit in `lm()` with a formula of the form `response~covariate+factor+covariated:factor` (*in that order to match what we did above*).^[Shorthand for fitting this model is `response~covariate*factor`.]
...
The parameter estimates and their confidence intervals are extractd with `coef()` and `confint()`, which can be bound together with `cbind()` to create a succinct summary.
...
The parameter estimates are under the "Ests" column and are in rows that are generally labeled as the variables in the ultimate full model. Do note that R prepends the indicator variable with the name of the original factor variable. Thus, what we called "COHO", R calls "speciescoho". From this then, $\hat{\alpha}$=`r formatC(cf.m1[1,"Ests"],format="f",digits=3)`, $\hat{\beta}$=`r formatC(cf.m1[2,"Ests"],format="f",digits=3)`, $\hat{\delta}_{1}$=`r formatC(cf.m1[3,"Ests"],format="f",digits=3)`, and $\hat{\gamma}_{1}$=`r formatC(cf.m1[4,"Ests"],format="f",digits=3)`. Thus, for example, the sample slope for Chinook Salmon (the reference groups) is `r formatC(cf.m1[2,"Ests"],format="f",digits=3)` and the sample slope for Coho Salmon is `r formatC(cf.m1[4,"Ests"],format="f",digits=3)` greater than the sample slope for Chinook Salmon. Furthermore, for example, one is 95% confident that $\beta$ is between `r formatC(cf.m1[2,"2.5 %"],format="f",digits=3)` and `r formatC(cf.m1[2,"97.5 %"],format="f",digits=3)` and $\gamma_{1}$ is between `r formatC(cf.m1[4,"2.5 %"],format="f",digits=3)` and `r formatC(cf.m1[4,"97.5 %"],format="f",digits=3)`.
...
From this plot, note that the intercept for Coho is less than the intercept for Chinook (as you would expect because $\hat{\delta}_{1}$=`r formatC(cf.m1[3,"Ests"],format="f",digits=3)`) and the slope for Coho is greater than (i.e., steeper) than that for Chinook (as you would expect because $\hat{\gamma}_{1}$=`r formatC(cf.m1[4,"Ests"],format="f",digits=3)`)
