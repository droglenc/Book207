...
Fitting ultimate full model in R is straightforward because it is essentially the same as fitting a Two-Way ANOVA with `lm()` (see \@ref(model-fitting-in-r)) and R automatically creates the indicator variables behind the scenes. The ultimate full model for an IVR is fit in `lm()` with a formula of the form `response~covariate+factor+covariate:factor` (*in that order to match what we did above*).^[Shorthand for fitting this model is `response~covariate*factor`.]
