...
In any one comparison of two means the probability of incorrectly concluding that the means are different when they are actually not different is &alpha;. This incorrect conclusion is called a **pairwise Type I error** because it relates to only one comparison of a pair of means.
...
::: {.tip data-latex=""}
A Type I error is rejecting H<sub>0</sub> when H<sub>0</sub> is actually true. In a two-sample t-test, a Type I error is concluding that two means are significantly different when they are not different.
:::
...
The `$emmeans` section contains the group sample means under `emmean` with 95% confidence intervals under `lower.CL` and `upper.CL`. For example, the sample mean immunoglobulin level for opossums in February was `r formatC(mc1$emmeans$emmean[1],format="f",digits=3)`, with a 95% confidence interval from `r formatC(mc1$emmeans$lower.CL[1],format="f",digits=3)` to `r formatC(mc1$emmeans$upper.CL[1],format="f",digits=3)`. The `t.ratio` and `p.value` in this section test if the group population mean is different than 0. These tests are not often of interest and can largely be ignored.

::: {.tip data-latex=""}
The group means with 95% confidence intervals are shown in the `$emmeans` section of the results.
:::

A plot of group means with 95% confidence intervals using the results in `$emmeans` is slightly different than shown in Sections \@ref(two-sample-t-test-revisited-using-linear-models) and \@ref(one-way-anova-in-r) because the raw data and the means with their confidence intervals are in separate data frames. While this method is slightly more complicated, it will generalize to a wider variety of situationst throughout the course.
