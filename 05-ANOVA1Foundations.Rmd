...
* Test if mean social capital differed among children from Estonia, Germany, and Russia ([Beilmann *et al.* (2014)](https://www.researchgate.net/publication/268977834_The_Relationship_between_Adolescents%27_Social_Capital_and_Individualism-Collectivism_in_Estonia_Germany_and_Russia)).
...
where $I$ is the total number of groups or populations.^[From this, it is evident that the one-way ANOVA is a direct extension of the [2-sample t-test](2TReview.html).] The the alternative hypothesis for a one-way ANOVA is "wordy" and is often written as

$$ \text{H}_{\text{A}}:\text{At least one pair of means is different} $$

because not all pairs of means need differ for the null hypothesis to be rejected. Thus, a rejection of H<sub>0</sub> in favor of H<sub>A</sub> is a statement that *some* difference in group means exists. It does not indicate which group means differ.^[Methods to identify which group means differ are in Module \@ref(ANOVA1MultipleComparisons).]

::: {.tip data-latex=""}
Rejecting H<sub>0</sub> **just** means that **some** group means differ.
:::

The simple ($Y_{ij} = \mu + \epsilon_{ij}$) and full ($Y_{ij} = \mu_{i} + \epsilon_{ij}$) models for the one-way ANOVA are the same as those for the [2-sample t-test](2TReview.html), except that $I$ may be >2 means in the full model. Thus, SS<sub>Total</sub>, SS<sub>Within</sub>, and SS<sub>Among</sub>; df<sub>Total</sub> (=$n$-1), df<sub>Within</sub> (=$n$-$I$). and df<sub>Among</sub> (=$I$-1); and MS, F, and p-values are all computed using the same formulae shown in Module \@ref(ModelComparison), except to again note that $I \geq$2.^[The MS, F, and p-value are computed the same in nearly every ANOVA table encountered in this class.]
...
SST <- ggplot(data=opp1) +
  geom_crossbar(mapping=aes(x=season,y=mn,ymin=mn,ymax=mn),color="transparent") +
...
An ANOVA table (Table \@ref(tab:oppANOVA)) is used to display the results from a one-way ANOVA, because the one-way ANOVA is simply a comparison of two models.
