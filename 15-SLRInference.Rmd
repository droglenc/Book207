...
The confidence band can be added to the scatterplot with the best-fit line (as shown in Section \@ref(best-fit-line-in-r)) by leaving off `se=FALSE` in `geom_smooth()`. Use `color=` to change the color of the best-fit line and `fill=` to change the color of the confidence band (if so desired).
