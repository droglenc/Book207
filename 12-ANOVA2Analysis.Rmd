...
### Main Effects
When an interaction is not present, as is the case here, multiple comparisons are conducted for factors related to any significant main effect. However, multiple comparisons on the main effects are compromised by the interaction term in the model. This is seen by the note from `emmeans()` below, where I tried to perform multiple comparisons for the density main effect with a model (`lm1`) that contains an interaction term.

```{r message=TRUE}
mc1 <- emmeans(lm1,specs=pairwise~density) # lm1 has an interaction term!!
```

This issue can be avoided by fitting a new model without the interaction term and then using that model for performing multiple comparisons. A model without an interaction term is fit using `response~factorA+factorB` in `lm()`. The ANOVA table is shown below to confirm that the interaction term is not in this new model.
